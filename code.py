...
file = open("C:/Users/ahmed/OneDrive/Desktop/Desktop/MASTER/Analysis&Design of Algorithms/Project/twitter.csv", "r")
accounts = {}
for x in set(file):
    two_acc = x.split(",")
...
while len(max_heap.heap) > 0:
    ...
    foll_cnt= accounts[max_key][0]
    foll_sum+=foll_cnt
    stand_div +=(foll_cnt-21)^2
    print(f"Account: {max_key}, Followers Number: {foll_cnt}")
...
print("[!] Average followers per account:",foll_sum//cnt)
print("[!] The standard deviation:",sqrt(stand_div/cnt))
end_time = time.time()
...
def Closest_Group(id,accounts,threshold):
    ...
    if threshold < 1:
        threshold_from_percent_to_num = len(target_acc_followers) * threshold
    close_group=[]
    for key,value in accounts.items():
        if key == id:
            continue
        ...
        if threshold >= 1:
            ...
            if cnt >= threshold_from_percent_to_num:
                close_group.append(key)
    return close_group
my_id = input("[!] Enter your ID to recommend new friends: ")
start_time = time.time()
print(Closest_Group(my_id,accounts,0.10))
end_time = time.time()
total_time = end_time - start_time
print(f"[!] The entire operation took just {total_time} seconds.")
